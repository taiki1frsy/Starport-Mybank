...
const baseQueryMyBalancesRequest = {};
export const QueryMyBalancesRequest = {
    ...
    decode(input, length) {
        const reader = input instanceof Uint8Array ? new Reader(input) : input;
        let end = length === undefined ? reader.len : reader.pos + length;
        const message = { ...baseQueryMyBalancesRequest };
        while (reader.pos < end) {
            const tag = reader.uint32();
            switch (tag >>> 3) {
                case 1:
...
const baseQueryMyBalancesResponse = {};
export const QueryMyBalancesResponse = {
    ...
    toJSON(message) {
        ...
        if (message.MyBalance) {
            obj.MyBalance = message.MyBalance.map((e) => e ? MyBalance.toJSON(e) : undefined);
        }
        else {
            obj.MyBalance = [];
        }
...
export class QueryClientImpl {
    ...
    Params(request) {
        ...
        const promise = this.rpc.request("taikifuru.mybank.mybank.Query", "Params", data);
